...
public class SudokuUtilities {
    ...
    private static void uniqueBoardSetup(int[][][] board) {
        Random rand = new Random();
        int[][] indexArray1  = new int[9][9];
        int[][] indexArray2  = new int[9][9];
        int number1 = rand.nextInt(1, 10);
        int number2 = rand.nextInt(1, 10);

        for(int side=0; side<2; side++) {
            for (int row = 0; row < GRID_SIZE; row++) {
                for (int col = 0; col < GRID_SIZE; col++) {
                    if (board[row][col][side] == number1)
                        indexArray1[row][col] =  number1;
                    if (board[row][col][side] == number2)
                        indexArray2[row][col] =  number2;
                }
            }
            for (int row = 0; row < GRID_SIZE; row++) {
                for (int col = 0; col < GRID_SIZE; col++) {
                    if (indexArray1[row][col] == number1)
                        board[row][col][side] =  number2;
                    if (indexArray2[row][col] == number2)
                        board[row][col][side] =  number1;
                }
            }
        }
    }
